 ...
 */
export const resolveYarnOptions = (gulpYarnOptions: CommandOptions): [ErrorOrNull, string[]] => {
  if (gulpYarnOptions) {
    let error = null;
    ...
    for (const [key, value] of Object.entries(otherOptions)) {
      if (key in Commands) {
        ...
        error = new PluginError(PLUGIN_NAME, `'${key}' option is not supported by the plugin. Please use 'args' option.`);
        return [error, []];
      }
    }
